...
| Input | Action |
|---|---|
| Drag a playback stream onto a sink | **Move** — WirePlumber `target.object` / `wpctl set-target`. Survives a track restart. |
...
| `n` | Spawn `Loom-<name>` null sink (`pactl load-module module-null-sink`) |
...
- **Virtual sinks** use one spawn path (`pactl` null-sink, `Loom-` prefix) and will not unload anything else. The day-3 stock-Omarchy proof was not run on this machine; the feature ships but teardown depends on the module id `pactl` returned.
...
- **`loomd` native subscribe** is optional. The product is the CLI path. A macOS checkout cannot link libpipewire; `build.sh` then produces a CLI-only binary or the POSIX wrapper.
